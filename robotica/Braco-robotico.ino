...
static unsigned long delayPausa;
  ...
  if (modo == 0) {
     ...
     if (pinBotao2Retencao()) {
        ...
        if (movimento == 0) {
          byte velocidade = map(analogRead(pinPot7),0,1023,255,0);  
          ...
          servo6.slowmove(readMemoria(posMemoria,5), velocidade);
          //servo6.slowmove( map(readMemoria(posMemoria,2), 0, 179, 179, 0), velocidade);  //Servo sincronizado inversamente ao Servo3    
          movimento = 1;
        } 
        ...
        if (movimento == 1) {
          if ( (servo1.read() == readMemoria(posMemoria,0)) &&
               ...
               (servo6.read() == readMemoria(posMemoria,5)) ) {
  ...
  if (modo == 1) {
     ...
     servo6.write( map(analogRead(pinPot6),0,1023,10,169) ); 
     //servo6.write( map(analogRead(pinPot3),0,1023,179,0) );  //Servo sincronizado inversamente ao Servo3    
     ...
     if (pinBotao2Apertado()) {
        ...
        setMemoria(ultMemoria, 5, map(analogRead(pinPot6),0,1023,0,179));
